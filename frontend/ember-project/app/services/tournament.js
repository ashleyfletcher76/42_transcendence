...
export default class TournamentService extends Service {
  ...
  handleLeave(parsedMessage) {
    ...
    this.currentPlayers = this.currentPlayers.filter(
      (player) => player.nickname !== parsedMessage.player,
    );
    const data = {
      type: 'tournament',
      from: 'System',
  ...
  handleMatch(parsedMessage) {
    ...
    const data = {
      ...
      from: 'System',
      content:
        'Get ready! Your next game is against ' +
        opponent +
        ' and it starts in just 20 seconds!',
    };
    this.chat.messages = [...this.chat.messages, data];
    const roomdata = {
  ...
  { 
	const data = {
		type: 'tournament',
		from: 'System',
		content: parsedMessage.winner + ' won the Tournament!',
	  };
	  this.chat.messages = [...this.chat.messages, data];
  }
